<template>
  <div>
    <Container v-model:x="character.x" v-model:y="character.y" :width="30" :height="20" :depth="character.y" @create="onCreate">
      <Image ref="image" :texture="`chara_sprite/${character.name}`" :frame :originX="0.5" :originY="1" :lighting="lighting" />
      <Body :velocityX="character.velocityX" :velocityY="character.velocityY" />
    </Container>
    <TapArea v-if="event" :visible="checkable" type="talk" :width="source.width + 15" :height="source.height + 40" :x="character.x" :y="character.y" @tap="onTap" />
  </div>
</template>
